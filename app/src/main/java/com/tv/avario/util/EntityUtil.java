...
import com.tv.avario.api.APIMultiListener;
...
public class EntityUtil {
   ...
   *
   * @param entityJSON
   * @return url value from icon_on / icon_off attributes contained in an array for easy integration
   * with AssetUtil loaders
   * @throws AvarioException when icon_off property cannot be found from within the entity
   */
  public static String[] getStateIconUrl(Context context, JSONObject entityJSON)
    ...
    if (!entityJSON.has(property)) { property = "icon_off"; }
    ...
    for (int index = 0, limit = apiJSON.length(); index < limit; index++) {
      ...
      try {
        try {
          ...
          if (requestJSON.has("logic") && !RefStringUtil.processLogic(
              requestJSON.getString("logic"))) { continue; }
        ...
        Application.mainHandler.post(
            new PlatformUtil.ErrorToastRunnable(context, exception));
      }
    }
    ...
    try {
      ...
      APIClient
          ...
          .sequenceRequests(
              ...
              entityId,
              entityId,
              new HttpListener(entityId, new String[]{entityId}, context)
          );
    } catch (final AvarioException exception) {
      Application.mainHandler.post(new PlatformUtil.ErrorToastRunnable(context, exception));
    }
  }
    ...
    public void run() {
      ...
      for (int index = 0, limit = apiJSON.length(); index < limit; index++) {
        ...
        try {
          ...
          Application.mainHandler.post(
              new PlatformUtil.ErrorToastRunnable(this.context, exception));
        }
      }
        ...
        public void run() {
          ...
          for (VolleyError error : self.errors) {
            ...
            if (error.networkResponse != null) {
              code = PlatformUtil.responseCodeToErrorCode("1",
