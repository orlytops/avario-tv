...
public class StateArray {
   ...
   * @return instance of the StateArray singleton or null if the filesystem cache does not exist yet
   */
  public static StateArray getInstance(Context context) {
    if (StateArray.instance == null) { StateArray.instance = new StateArray(context); }
  ...
  public StateArray broadcastRoomChange(final JSONObject payload) {
    ...
    try {
      ...
      if (!BluetoothScanner.getInstance().isEnabled() ||
          this.getRoom(roomId).optBoolean("active", false)) { return this; }

      try {
        if (!payload.getJSONObject("event_data").getString("tablet_id")
            .equals(PlatformUtil.getTabletId())) { return this; }
  ...
  /**
   * Parse {"event_type": "bootstrap_changed","event_data": {"tablet_id": "00:11:22:333:44:55"}}
   * payload
   *
   * @param payloadJSON
   */
    ...
    for (String property : properties) {
      ...
      while (entities.hasNext()) {
        ...
        try {
          ...
          if (entityJSON.has("new_state")) { entityJSON.put("new_state", new JSONObject()); }

          if (entityJSON.has("old_state")) { entityJSON.put("old_state", new JSONObject()); }
        } catch (JSONException exception) {
        }
      }
    }
  ...
  public JSONArray getTelevisionItems() throws AvarioException {
    if (!this.hasData()) { return null; }

    try {
      return this.data
          .getJSONObject("television")
      ...
      throw new AvarioException(
          ...
          exception,
          new Object[]{
              String.format("television.%s", "elements")
          }
      );
    }
  }
  ...
  public JSONObject getDial(String dialId) throws AvarioException {
    if (!this.hasData() || dialId == null) { return null; }
  ...
  public JSONObject getDialButton(String dialbuttonId) throws AvarioException {
    if (!this.hasData() || dialbuttonId == null) { return null; }
  ...
  public JSONObject getDialButtonState(String stateId) throws AvarioException {
    if (!this.hasData() || stateId == null) { return null; }
  ...
  public String updateFromMQTT(JSONObject mqttJSON) throws AvarioException {
    ...
    try {
      ...
      if (!mqttJSON.getString("event_type").equalsIgnoreCase(
          Constants.MQTT_EVENT_TYPE_STATE_CHANGED)) {
      ...
      for (String key : new String[]{"new_state", "old_state"}) {
        ...
        if (eventJSON.isNull(key)) { continue; }
    ...
    for (int index = 0, length = httpJSON.length(); index < length; index++) {
      ...
      if (entityJSON != null) {
        try {
          ...
          { this.updateState("old_state", updateJSON); } else {
  ...
  public StateArray save() throws AvarioException {
    ...
    if (this.data == null || !this.dirty) { return this; }
