package com.tv.avario.fragment;
...
import com.tv.avario.BuildConfig;
...
import com.tv.avario.service.AvarioReceiver;
import com.tv.avario.util.AssetLoaderTask;
import com.tv.avario.util.AssetUtil;
import com.tv.avario.util.Connectivity;
import com.tv.avario.util.DrawableLoader;
import com.tv.avario.util.Log;
import com.tv.avario.util.SystemUtil;
import com.tv.avario.util.Validator;
...
//                .setNeutralButton(R.string.setting_get_new_bootstrap, null);
  ...
  public void onDetach() {
    ...
    this.config.restore();
    if (this.listener != null) { this.listener.onDialogDetached(); }
  ...
  private View setupViews(LayoutInflater inflater, ViewGroup container) {
    ...
    kioskCheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
      ...
      public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (isChecked) {
          ...
          getActivity().getPackageManager().clearPackagePreferredActivities(
    ...
    if (this.config.isSet()) {
      ...
      relativeLayoutCache.setVisibility(View.GONE);
      bootstrapSource.setText(
          "Source: " + this.config.getHttpHost() + ":" + this.config.getHttpPort());
    }
   ...
   * @return true if the app needs an update false if the app is up to date
   */
    ...
    try {
      ...
      if (!version.equals(currentVersionName)) {
        ...
        for (int i = 0; i < maxLength; i++) {
          ...
          try {
            if (Integer.parseInt(storeVersionCodeArray[i]) > Integer.parseInt(
                currentVersionCodeArray[i])) {
              updateNeeded = true;
              continue;
            }
  ...
  private void applySnapshot() {
    if (this.snapshot.initial) { this.config.clear(); } else {
  ...
  private void toggleError(boolean show, AvarioException exception) {
    ...
    if (exception == null) { message = ""; } else {
  ...
  private boolean deleteAssetCache(File dir) {
    ...
    if (dir != null && dir.isDirectory()) {
      ...
      for (String child : children) { deleteAssetCache(new File(dir, child)); }
...
//            self.refreshB = button;
      ...
      if (view.getId() == R.id.btnSave) {
        ...
        noneFatalMessage.setText("Test firebase");
                /*if (noneFatalMessage.getText() == null
                    ...
                    Toast.makeText(view.getContext(), "None Fatal Message should not be empty",
                    Toast.LENGTH_SHORT).show();
                } else {
                    FirebaseCrash.report(new Throwable(noneFatalMessage.getText().toString()));
                    Toast.makeText(view.getContext(), "None Fatal crash " + noneFatalMessage
                    .getText().toString()
  ...
  private class MqttConnectionListener implements MqttConnection.Listener {
    ...
    public void onSubscription(MqttConnection connection) {
      ...
      if (self.listener != null) { self.listener.onSettingsChange(); }
  ...
  private void setDefaultCosuPolicies(boolean active) {
    ...
    mDevicePolicyManager.setLockTaskPackages(mAdminComponentName,
        active ? new String[]{getActivity().getPackageName(), "com.google.android.youtube"}
            : new String[]{});
